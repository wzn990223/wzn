#定义了一个用于二分类的基本卷积神经网络，可以通过图像反应损失下降过程，观察训练结果好坏
# 并且每50次训练都会将模型参数保存在ckpt文件中，在下次计算时可以直接加载。
import os
...
def creat_x_database(rootdir,resize_row,resize_col):
    ...
    for i in range(0, len(list)):
        ...
        if os.path.isfile(path):                ##判断路径是否为文件
            image_raw_data = tf.gfile.GFile(path,'rb').read()#读取图片
...
# 创建训练集
fail_x_data = creat_x_database('D:\jiance\zFalse', 40, 40)
true_x_data = creat_x_database('D:\jiance\zTure', 40, 40)
...
xs = tf.placeholder(tf.float32, [None, 40, 40, 3]) / 255  # 归一化
ys = tf.placeholder(tf.float32, [None, 2])
#xs = tf.placeholder(tf.float32, [1, 128, 128, 3]) / 255  # 归一化
...
gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.8)#定义GPU使用率、使用GPU加速计算
...
for i in range(1000):
    ...
    if i % 50 == 0:
        ...
        i_v.append(i)
        c_e.append(sess.run(cross_entropy,feed_dict={xs: x_train, ys: y_train, keep_prob: 1}))
        saver.save(sess,"D:/jiance/save/model/model.ckpt")#定义模型参数保存路径
        tt2 = time()
        tt = tt2 - tt1
        print('time=%.5f' % tt)
#画图显示损失下降过程
